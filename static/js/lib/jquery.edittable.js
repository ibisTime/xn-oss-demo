    ...
    $.fn.editTable = function (options) {
        ...
        function buildCell(content, type) {
            ...
            if (type && 'text' !== type){
                ...
                var rowLength = $table.find('tbody tr').length+1;
                var fieldHtml = field.html;
                if(fieldHtml.indexOf('firstControl') != -1){
                	fieldHtml = fieldHtml.replace('firstControl', rowLength+'1');
                }else if(fieldHtml.indexOf('secondControl') != -1){
                    fieldHtml = fieldHtml.replace('secondControl', rowLength+'11');
                }
                return '<td>' + field.setValue(fieldHtml, content)[0].outerHTML + '</td>';
            }
            // Default
