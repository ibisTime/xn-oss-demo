import { getOwnerBtns, getMenuList, getPageMenus } from 'api/general';

const PREFIX = 'SECURITY_MENU_';
...
export function securityMenu(state = initState, action) {
  switch(action.type) {
...
export function getInitData(parentCode) {
  return (dispatch, getState) => {
    dispatch(fetching());
    let pagination = getState().securityMenu.pagination;
    ...
    Promise.all([
      ...
      getPageMenus({
        current,
        pageSize,
        ...getState().securityMenu.searchParam
      }),
      getMenuList()
    ]).then(([btnData, tableData, menuData]) => {
      ...
      dispatch(setSearchData({ data: menuData, key: 'parentCode' }));
    }).catch(msg => {
      dispatch(cancelFetching());
...
export function getPageTableData(current, searchParam) {
  return (dispatch, getState) => {
    ...
    if (searchParam) {
      ...
      searchParam = getState().securityMenu.searchParam;
    }
    getPageMenus({
      current,
      pageSize,
